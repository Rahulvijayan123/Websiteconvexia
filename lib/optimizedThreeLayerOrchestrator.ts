...
class OptimizedThreeLayerOrchestrator {
  ...
  async orchestrate(inputs: any): Promise<OptimizedResult> {
    ...
    try {
      ...
      let currentOutput = await this.executeLayer1Research(inputs);
      ...
      // Continue with existing validation cycles
      ...
      while (cycleCount < this.config.maxValidationCycles) {
        ...
        // Check cost limits
        if (globalCostTracker.getRemainingBudget() <= 0.5) {
          console.log('💰 Cost limit approaching, stopping validation cycles');
          break;
        }
      }

      // Layer 3: GPT-4o Enhancement
      let gpt4oEnhancement = null;
      if (this.config.enableGPT4oEnhancement) {
        console.log('🧠 Layer 3: GPT-4o Enhancement (Logic, Math, Writing)');
        try {
      ...
      console.error('❌ Optimized three-layer orchestration failed:', error);
      throw error;
    }
  }
...
Return completely regenerated JSON that addresses ALL validation issues and follows ALL business logic rules exactly.`;
  ...
  private async executeRegenerationWithValidation(prompt: string): Promise<any> {
    ...
    const model = globalCostTracker.getOptimalModel('research', globalCostTracker.getRemainingBudget());
    const response = await fetchPplx({
      model,
      messages: [{ role: 'user', content: prompt }],
      response_format: { 
        type: 'json_schema',
        json_schema: this.getCommercialSchema()
      }
    });
