...
- For dealActivity:
  ...
  - For each deal, you MUST use at least 50 unique, up-to-date web searches to find and include ALL of the following fields in the output JSON for every deal object: asset name, development stage at deal time, deal price (value), deal date, and buyer/acquirer. These fields are required and must be present in the output for every deal; if any field cannot be found after exhaustive searching, you must output 'Unknown' for that field, and explain in the research log exactly what you searched and why the field could not be found. The output JSON for each deal must always include all these fields, even if some are 'Unknown'. This is non-negotiable.
